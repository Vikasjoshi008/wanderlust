...
module.exports.searchinglistrender = async (req, res) => {
  ...
  const filteredWords = searchWords.filter((word) => !skipWords.includes(word));

  try {
    const queryConditions = filteredWords.map((word) => {
    ...
    return res.render("listings/searchlist.ejs", {
      listing: searchlistingsfound,
      searchQuery: query,
    });
  } catch (err) {
    console.error(err);
